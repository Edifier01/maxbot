...
def test_production_lockfiles_match_python_runtime():
    root = Path(__file__).resolve().parents[1]
    for name in ("requirements.lock", "requirements-server.lock"):
        assert "autogenerated by pip-compile with Python 3.12" in (
            root / name
        ).read_text(encoding="utf-8")


def test_request_log_adds_correlation_id_without_request_data(caplog):
    ...
    with caplog.at_level("INFO", logger="maxsender.access"):
...
def test_webhook_allowlist_fails_closed_and_rejects_private_urls(monkeypatch):
    ...
    assert not webhook_url_allowed("https://hooks.example.com/event")
    monkeypatch.setenv("WEBHOOK_ALLOWED_HOSTS", "hooks.example.com")
    assert webhook_url_allowed("https://hooks.example.com/event")
    assert not webhook_url_allowed("http://hooks.example.com/event")
