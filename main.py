    ...
    async def authenticate(self, app) -> Any:
        ...
        if result.password_challenge:
            ...
            if not app.config.registration_config:
                raise RuntimeError("Для регистрации нового аккаунта нужна конфигурация RegistrationConfig")
...
async def _with_client(
    ...
    *,
    ...
    try:
        ...
        extra = ExtraConfig(**extra_kwargs)
        if proxy:
            append_log("Внимание: ExtraConfig не поддерживает прокси — работаем без него")
...
async def _worker_loop() -> None:
    ...
    while True:
        ...
        groups = _active_groups()
        if not messages:
            append_log("Нет сообщений — загрузите active.txt")
...
async def api_vault_lock():
    if vault_status()["legacy"]:
        raise HTTPException(
            400,
            "Legacy-ключ нельзя заблокировать — сначала защитите хранилище паролем",
        )
...
async def add_group(body: GroupIn):
    if not body.max_chat_id and not body.invite_link:
        raise HTTPException(400, "Укажите ID чата или ссылку-приглашение")
    with _conn() as c:
        cur = c.execute(
            "INSERT INTO groups (name, max_chat_id, invite_link, proxy) VALUES (?, ?, ?, ?)",
            (
                body.name,
                body.max_chat_id.strip(),
                body.invite_link.strip(),
                (body.proxy or "").strip(),
            ),
        )
        gid = cur.lastrowid
