...
def _ensure_group_role_plan(group_id: int) -> None:
    ...
    with m._conn() as c:
        rows = c.execute(
            """
            SELECT gp.profile_id, gp.role_day, gp.day_order, gp.is_enabled, p.status
        ...
        if not rows:
            return
        today_rows = [r for r in rows if r["role_day"] == today]
        if today_rows:
            # A member added, re-enabled, or re-authenticated after today's
            # plan was pinned is a late arrival. Do not recompute existing
            # roles or create replacement slots for them during this day.
            late_rows = [r for r in rows if r["role_day"] != today]
            if not late_rows:
                return
            next_order = max(
                (int(r["day_order"] or 0) for r in today_rows),
                default=-1,
            ) + 1
            for offset, row in enumerate(late_rows):
                c.execute(
                    "UPDATE group_profiles SET role_day=?, day_role='skip', day_order=? "
                    "WHERE group_id=? AND profile_id=?",
                    (today, next_order + offset, group_id, row["profile_id"]),
                )
            ...
            message = (
                f"Роли дня «{gname}»: сохранены текущие snapshots; "
                f"поздних участников={late_count} (до следующего дня skip)"
            )
        else:
